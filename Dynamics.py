...
<Attribute name="hsapiens_homolog_associated_gene_name"/>
        ...
        try:
            ...
            if text.startswith("Query ERROR") or "ERROR" in text[:500]:raise RuntimeError(text[:500])
        ...
        except Exception as exc:print(f"  BioMart batch {start//500+1} failed: {exc}")
    if rows:
        ...
        if ds in PLATFORMS:
            ...
            if ds=="GSE67462":
                ...
                if id_col is None or ref_col is None:
                    ...
                    platform=pd.DataFrame({"feature_id":table[id_col].astype(str).str.strip(),"refseq_mrna":table[ref_col].astype(str).str.strip()}); platform=platform[platform.refseq_mrna.ne("")&platform.refseq_mrna.ne("nan")].drop_duplicates(); merged=expr.reset_index(names="feature_id").merge(platform,on="feature_id",how="left"); bm=biomart_refseq_to_human(merged.refseq_mrna.dropna().astype(str).tolist()); merged=merged.merge(bm[["refseq_mrna","human_gene"]],on="refseq_mrna",how="left"); audit=merged[["feature_id","human_gene"]].rename(columns={"human_gene":"gene_symbol"}); audit["dataset"]=ds; audit["mapped"]=audit.gene_symbol.notna(); mapped=merged.dropna(subset=["human_gene"]); sample_cols=[c for c in expr.columns if c in mapped.columns]; gene=mapped.groupby("human_gene")[sample_cols].mean()
