...
STAGE21=OUT/"stage2_1"; STAGE22=OUT/"stage2_2"; STAGE23=OUT/"stage2_3"; STAGE24=OUT/"stage2_4"
for d in (STAGE21,STAGE22,STAGE23,STAGE24): d.mkdir(parents=True,exist_ok=True)
...
def stage2_2_validate_common_latent(st):
    ...
    p=pd.DataFrame(pairs); p["path_length_ratio"]=p.path_length_a/p.path_length_b; p.to_csv(STAGE22/"02_cross_dataset_distances.csv",index=False); return p
def _branches(st):
...
def stage2_3_within_time_residual_validation(st):
    ...
    for d,c in branches:
        ...
        if curve is not None:trans[(d,c)]=_fit_transform(curve,refcurve) if not (d==ref and c=="all") else (np.eye(3),curve.mean(0),np.ones(3),refcurve.mean(0))
    out=st.copy()
    ...
    for i,(a,ca) in enumerate(branches):
        for b,cb in branches[i+1:]:
            ...
            rawcorr=np.corrcoef(xa.ravel(),xb.ravel())[0,1]; aa=_apply_transform(xa,_fit_transform(xa,xb)); alignedcorr=np.corrcoef(aa.ravel(),xb.ravel())[0,1]; rows.append({"trajectory_a":f"{a}:{ca}","trajectory_b":f"{b}:{cb}","raw_trajectory_correlation":rawcorr,"post_procrustes_correlation":alignedcorr,"correlation_gain":alignedcorr-rawcorr,"raw_rmse":np.sqrt(np.mean((xa-xb)**2)),"post_procrustes_rmse":np.sqrt(np.mean((aa-xb)**2))})
    ...
    ratio=pd.DataFrame(between); ratio.to_csv(STAGE23/"05_between_vs_within_distance.csv",index=False); return out,pd.DataFrame({"metric":["reference_dataset","trajectory_branches","mean_within_time_residual","median_within_time_residual","mean_correlation_gain_from_alignment","mean_between_to_within_ratio"],"value":[ref,";".join(f"{d}:{b}" for d,b in branches),sm.mean_within_time_residual_norm.mean(),sm.median_within_time_residual_norm.median(),gain.correlation_gain.mean(),ratio.between_to_within_ratio.mean()]})
def stage2_4_out_of_sample_validation(st):
    ...
    for d,b in branches:
        ...
        for hold in times:
            ...
            for rd,rb in branches:
                ...
                trans=_fit_transform(target_train,refcurve); pred_ref=np.array([np.interp(u,grid,refcurve[:,j]) for j in range(3)]); xheld=_point_at_time(st,d,hold,None if b=="all" else b)
                if xheld is None:continue
                pred=_apply_transform(xheld.reshape(1,-1),trans)[0]; err=float(np.linalg.norm(pred-pred_ref)); scale=float(np.linalg.norm(pred_ref-refcurve.mean(0)))
    ...
    o=pd.DataFrame(rows); o.to_csv(STAGE24/"01_leave_one_timepoint_out.csv",index=False); tested=o[o.status=="tested"].copy(); summary=[]
    ...
    sm=pd.DataFrame(summary); sm.to_csv(STAGE24/"02_oos_summary_by_trajectory.csv",index=False); base=[]
    for _,r in tested.iterrows():
        rd,rb=r.reference_trajectory.split(":",1); refcurve=_curve(st,rd,grid,None if rb=="all" else rb); u=r.normalized_time; refpt=np.array([np.interp(u,grid,refcurve[:,j]) for j in range(3)]); td,tb=r.target_trajectory.split(":",1); xheld=_point_at_time(st,td,r.held_out_time_hours,None if tb=="all" else tb); rawerr=float(np.linalg.norm(xheld-refpt)); base.append({"target_trajectory":r.target_trajectory,"held_out_time_hours":r.held_out_time_hours,"reference_trajectory":r.reference_trajectory,"aligned_prediction_error":r.prediction_rmse,"naive_unaligned_error":rawerr,"error_reduction":rawerr-r.prediction_rmse,"relative_error_reduction":(rawerr-r.prediction_rmse)/(rawerr+1e-8)})
    bd=pd.DataFrame(base); bd.to_csv(STAGE24/"03_oos_vs_naive_baseline.csv",index=False); report=pd.DataFrame({"metric":["tested_rows","target_trajectories_tested","mean_oos_error","median_oos_error","mean_relative_error","mean_error_reduction_vs_naive"],"value":[len(tested),tested.target_trajectory.nunique(),tested.prediction_rmse.mean() if not tested.empty else np.nan,tested.prediction_rmse.median() if not tested.empty else np.nan,tested.prediction_relative_error.mean() if not tested.empty else np.nan,bd.error_reduction.mean() if not bd.empty else np.nan]}); report.to_csv(STAGE24/"04_stage24_decision_metrics.csv",index=False)
    with open(STAGE24/"REPORT.txt","w",encoding="utf-8") as f:f.write("Dynamics v4.7 Stage 2.4 — out-of-sample trajectory validation\n\nA complete biological timepoint is held out from each target trajectory with >=3 timepoints. The transform is estimated without the held-out target point. Two-timepoint branches are marked insufficient.\n")
    return o,report
...
def stage5_critical_transitions(d):
    ...
    for _,idx in out.groupby(["dataset","trajectory_branch"]).groups.items():
        ...
        if w>=3:out.loc[q.index,"rolling_variance_latent1"]=s.rolling(w,min_periods=3).var().to_numpy(); out.loc[q.index,"rolling_autocorrelation_latent1"]=s.rolling(w,min_periods=3).apply(lambda z:z.autocorr(1) if z.std()>0 else np.nan).to_numpy()
...
def print_stage24(v):
    ...
    if v is None or v.empty:print("No Stage 2.4 results.")
        ...
        tested=v[v.status=="tested"] if "status" in v.columns else pd.DataFrame(); print("\n--- INTERPRETATION ---"); print(f"tested rows = {len(tested)}"); print(f"tested target trajectories = {tested.target_trajectory.nunique() if not tested.empty else 0}"); print("NOTE: Stage 2.4 is a generalization test. A low in-sample alignment error is not sufficient; held-out timepoints must also be predicted with low error."); print("Two-timepoint branches are intentionally excluded from the OOS test.")
    print("="*88)
def main():
    st,av=stage1_data_integration(); st=stage2_1_common_latent_state(st); stage2_2_validate_common_latent(st); st,v23=stage2_3_within_time_residual_validation(st); v24=stage2_4_out_of_sample_validation(st); print_stage24(v24); tr=stage3_trajectory_reconstruction(st); dy=stage4_dynamics(tr); dy=stage5_critical_transitions(dy); s=stage6_symbolic_equation_discovery(dy); h=stage7_heldout_validation(dy); r=stage8_regulatory_integration(dy); p=stage9_predictive_ai(dy)
    print(f"\nDynamics v4.7 results written to: {OUT}"); print(f"Datasets with PCA: {int(av.PCA_file_found.sum())}/{len(av)}"); print(f"Stage 2.1 aligned trajectory datasets: {st[st.common_latent_status=='time_anchored_aligned'].dataset.nunique()}"); print(f"Stage 2.4 tested OOS rows: {int((v24[0].status=='tested').sum())}"); print(f"Trajectory timepoints: {len(tr)}"); print(f"Stage 6 symbolic rows: {len(s)}"); print(f"Stage 7 held-out datasets: {len(h)}"); print(f"Stage 8 regulatory rows: {len(r)}"); print(f"Stage 9 prediction pairs: {len(p)}")
