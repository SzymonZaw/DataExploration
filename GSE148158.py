...
save_plot(fig, "04_sample_correlation.png")

log_expr = np.log2(expr.clip(lower=0) + 1)
variance = log_expr.var(axis=1).sort_values(ascending=False)
variance.to_csv(OUT / "05_feature_variance.csv", header=["variance"])
...
coords.to_csv(OUT / "06_PCA_coordinates.csv")
if len(EV) >= 2:
    ...
    ax.set_xlabel(f"PC1 ({EV[0]*100:.1f}%)"); ax.set_ylabel(f"PC2 ({EV[1]*100:.1f}%)"); ax.set_title("GSE148158 - PCA")
    save_plot(fig, "07_PCA.png")
...
save_plot(fig, "08_top_variable_features.png")

report = ["Dataset: GSE148158", f"Features: {expr.shape[0]:,}", f"Samples: {expr.shape[1]:,}", "Input: normalized counts"]
if len(EV) >= 2:
