...
def load_inputs():
    ...
    missing = [c for c in sample_cols if c not in set(meta[meta_sample_col].astype(str))]
    extra = [c for c in meta[meta_sample_col].astype(str) if c not in set(sample_cols)]
...
def map_to_hgnc(ids):
    ...
    for i in range(0, len(clean), 1000):
        result = info.querymany(
            clean[i:i + 1000], scopes="ensembl.gene", fields="symbol",
            species="human", as_dataframe=False, returnall=False,
            step=1000, verbose=False,
        )
        for row in result:
            if row.get("notfound"):
                continue
...
def pathway_scores(expr_hgnc: pd.DataFrame):
    """Score only the three predeclared nuisance families using decoupler."""
    import decoupler as dc

    progeny = dc.op.progeny(organism="human", top=100)
    scores = dc.mt.ulm(expr_hgnc, progeny, source="source", target="target", weight="weight", verbose=False)
    # decoupler returns a dataframe indexed by sample in recent versions.
    if isinstance(scores, tuple):
        scores = scores[0]
    if not isinstance(scores, pd.DataFrame):
        scores = pd.DataFrame(scores)
    scores.index = scores.index.astype(str)
    cols = [c for c in ["JAK-STAT", "NFkB", "NFkB1"] if c in scores.columns]
    return scores.loc[:, cols].copy() if cols else pd.DataFrame(index=expr_hgnc.index)
...
def main():
    ...
    report = {
        ...
        "input_hashes": {
            "GSE129486_gene_tpm": {"sha256": sha256(EXPR), "expected": EXPECTED_EXPR_SHA, "matches": sha256(EXPR) == EXPECTED_EXPR_SHA},
            "GSE129486_metadata": {"sha256": sha256(META), "expected": EXPECTED_META_SHA, "matches": sha256(META) == EXPECTED_META_SHA},
        },
    ...
    report["mapping"] = {
        ...
        "mapped_ids": int(mapping.__len__()),
        "unique_hgnc_symbols": int(mapped.shape[1]),
        "mapping_rate": float(len(mapping) / len(set(normalize_ensembl(expr[gene_col])))),
        "duplicate_symbols_aggregated": int(len(set(symbols.dropna())) - mapped.shape[1] + 0),
    }

    # The pathway audit is deliberately descriptive. We do not threshold activity
    # here; activity distributions and metadata stratification are persisted so
    # the final orthogonality rule can be specified prospectively.
    ...
    # A direct IRF/STAT TF activity score is not silently substituted for the
    # predeclared H2 definition: the exact DoRothEA activity output needs to be
    # evaluated in the same representation contract in a follow-up implementation.
