...
class BenchmarkConfig:
    max_genes:int=2000; state_dim:int=8; hidden_dim:int=128; epochs:int=250; lr:float=1e-3; prefix_fraction:float=.6; history_len:int=2; history_dim:int=16; dropout:float=.05; permutation_n:int=1000
...
def training_statistics(train,max_genes):
    arrays=[X for _,X in train.values()];raw=np.vstack(arrays);med=np.nanmedian(np.where(np.isfinite(raw),raw,np.nan),axis=0);med=np.where(np.isfinite(med),med,0.0);pooled=np.vstack([np.where(np.isfinite(X),X,med) for X in arrays]);var=np.var(pooled,axis=0);keep=np.argsort(var)[::-1][:min(max_genes,pooled.shape[1])];mean=pooled[:,keep].mean(0);std=np.where(pooled[:,keep].std(0)>1e-8,pooled[:,keep].std(0),1.0);return keep,mean,std
...
def train_dynamic(train_data,keep,mean,std,cfg,seed,mode):
    set_seed(seed);hl=cfg.history_len if mode=="memory" else 0;xa,ya,dta,ha=chronological_examples(train_data,keep,mean,std,hl);x=torch.tensor(xa,dtype=torch.float32);y=torch.tensor(ya,dtype=torch.float32);dt=torch.tensor(dta,dtype=torch.float32);context_dim=1 if mode in {"delta_t","memory"} else 0;history_dim=cfg.history_dim if mode=="memory" else 0;model=DynamicStateModel(x.shape[1],cfg.state_dim,cfg.hidden_dim,context_dim,history_dim,cfg.dropout);opt=torch.optim.AdamW(model.parameters(),lr=cfg.lr,weight_decay=1e-4);best=float("inf");stale=0;patience=max(25,cfg.epochs//8)
...
def train_autoencoder(train_data,keep,mean,std,cfg,seed):
    set_seed(seed);xa,ya,_,_=chronological_examples(train_data,keep,mean,std,0);x=torch.tensor(xa,dtype=torch.float32);y=torch.tensor(ya,dtype=torch.float32);model=StaticAutoencoder(x.shape[1],cfg.state_dim,cfg.hidden_dim,cfg.dropout);opt=torch.optim.AdamW(model.parameters(),lr=cfg.lr,weight_decay=1e-4)
...
def _metrics(true,pred,persistence,nearest,linear):
    rm=lambda a,b:float(np.sqrt(np.mean((np.asarray(a)-np.asarray(b))**2)));out={"n_future_points":len(true),"rmse_model":rm(true,pred),"rmse_persistence":rm(true,persistence),"rmse_nearest":rm(true,nearest) if np.isfinite(nearest).all() else np.nan,"rmse_linear":rm(true,linear) if np.isfinite(linear).all() else np.nan,"_true":np.asarray(true),"_pred":np.asarray(pred),"_persistence":np.asarray(persistence)};out["improvement_vs_persistence"]=out["rmse_persistence"]-out["rmse_model"];out["improvement_vs_nearest"]=out["rmse_nearest"]-out["rmse_model"] if np.isfinite(out["rmse_nearest"]) else np.nan;out["improvement_vs_linear"]=out["rmse_linear"]-out["rmse_model"] if np.isfinite(out["rmse_linear"]) else np.nan;return out

def evaluate_dynamic(model,train_data,heldout_data,keep,mean,std,cfg,mode,order=None):
    times,Xraw=heldout_data;X=transform(Xraw,keep,mean,std);order=np.arange(len(times)) if order is None else np.asarray(order);times_o=times[order];X_o=X[order];n_prefix=min(max(2,cfg.history_len+1,int(np.ceil(len(times_o)*cfg.prefix_fraction))),len(times_o)-1);observed=[v.copy() for v in X_o[:n_prefix]];current=observed[-1].copy();current_t=float(times_o[n_prefix-1]);true=[];pred=[];persistence=[];nearest=[];linear=[];train_trans={k:transform(v,keep,mean,std) for k,(_,v) in train_data.items()};scale=max(float(times[-1]-times[0]),1.0);prefix_t=list(times_o[:n_prefix])
    for j in range(n_prefix,len(times_o)):
        dt=(float(times_o[j])-current_t)/scale;hist=np.asarray(observed[-cfg.history_len:])[None,:,:] if mode=="memory" else None;p=predict_dynamic(model,current,dt,hist,mode);pred.append(p);persistence.append(observed[-1]);true.append(X_o[j]);nearest.append(np.mean([train_trans[ds][int(np.argmin(np.abs(tt-times_o[j])))] for ds,(tt,_) in train_data.items()],axis=0));linear.append(observed[-1]+((times_o[j]-prefix_t[-1])/(prefix_t[-1]-prefix_t[-2]))*(observed[-1]-observed[-2]) if len(prefix_t)>=2 and prefix_t[-1]!=prefix_t[-2] else observed[-1]);current=p;current_t=float(times_o[j]);observed.append(p)
...
def evaluate_autoencoder(model,transition,heldout_data,keep,mean,std,cfg,order=None):
    times,Xraw=heldout_data;X=transform(Xraw,keep,mean,std);order=np.arange(len(times)) if order is None else np.asarray(order);X=X[order];n_prefix=min(max(2,int(np.ceil(len(times)*cfg.prefix_fraction)),len(times)-1),len(times)-1);Z=model.encode(torch.tensor(X,dtype=torch.float32)).detach().numpy();true=Z[n_prefix:];pred=[];cur=Z[n_prefix-1].copy()
...
def evaluate_pca(train_data,heldout_data,keep,mean,std,cfg,order=None):
    train_z=np.vstack([transform(X,keep,mean,std) for _,X in train_data.values()]);scaler=StandardScaler().fit(train_z);pca=PCA(n_components=min(cfg.state_dim,train_z.shape[0],train_z.shape[1])).fit(scaler.transform(train_z));times,Xraw=heldout_data;X=transform(Xraw,keep,mean,std);order=np.arange(len(times)) if order is None else np.asarray(order);Z=pca.transform(scaler.transform(X[order]));n_prefix=min(max(2,int(np.ceil(len(times)*cfg.prefix_fraction)),len(times)-1),len(times)-1);true=Z[n_prefix:];pers=np.repeat(Z[n_prefix-1][None,:],len(true),axis=0);linear=[]
    for j in range(n_prefix,len(times)):linear.append(Z[n_prefix-1]+((times[order[j]]-times[order[n_prefix-1]])/(times[order[n_prefix-1]]-times[order[n_prefix-2]]))*(Z[n_prefix-1]-Z[n_prefix-2]) if times[order[n_prefix-1]]!=times[order[n_prefix-2]] else Z[n_prefix-1])
...
def benchmark_fold(train_data,heldout_data,cfg,seed,heldout_name):
    ...
    for mode in ("pca","autoencoder","markov","delta_t","memory"):
        ...
        elif mode=="autoencoder":model,transition=train_autoencoder(train_data,keep,mean,std,cfg,seed);r=evaluate_autoencoder(model,transition,heldout_data,keep,mean,std,cfg)
        else:model,loss=train_dynamic(train_data,keep,mean,std,cfg,seed,mode);r=evaluate_dynamic(model,train_data,heldout_data,keep,mean,std,cfg,mode);r["train_loss"]=loss
        runs[mode]={"model":mode,"seed":seed,"heldout_dataset":heldout_name,"train_data":train_data,"heldout_data":heldout_data,"keep":keep,"mean":mean,"std":std,"cfg":cfg,"fitted":(model,transition) if mode=="autoencoder" else model,"observed":r};rows.append({"seed":seed,"heldout_dataset":heldout_name,"model":mode,"n_selected_genes":len(keep),**{k:v for k,v in r.items() if not k.startswith("_")}})
    return rows,runs
