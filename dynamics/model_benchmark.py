...
def train_neural(train_data,keep,mean,std,cfg,seed,mode):
    ...
    x,y,dt,h=chronological_examples(train_data,keep,mean,std,hist_len); context_dim=1 if mode in {"delta_t","memory"} else 0; history_dim=cfg.history_dim if mode=="memory" else 0
    ...
    for _ in range(cfg.epochs):
        ...
        pred=model.transition(z,context=dt if context_dim else None,history=h if history_dim else None);loss=((rec-x)**2).mean()+.25*((pred-zn)**2).mean()+((model.decode(pred)-y)**2).mean();loss.backward();nn.utils.clip_grad_norm_(model.parameters(),1.0);opt.step()
...
def predict_step(model,current,dt,history,mode):
    x=torch.tensor(current[None,:],dtype=torch.float32);ctx=torch.tensor([[float(dt)]],dtype=torch.float32) if mode in {"delta_t","memory"} else None;h=torch.tensor(history[None,:,:],dtype=torch.float32) if mode=="memory" else None
    model.eval()
...
def evaluate_neural(model,train_data,heldout_data,keep,mean,std,cfg,mode):
    ...
    for j in range(n_prefix,len(times)):
        dt=(float(times[j])-current_t)/scale;hist=np.asarray(observed[-cfg.history_len:]) if mode=="memory" else None;p=predict_step(model,current,dt,hist,mode)
        ...
        linear.append(observed[-1]+((times[j]-prefix_t[-1])/(prefix_t[-1]-prefix_t[-2]))*(observed[-1]-observed[-2]) if len(prefix_t)>=2 and prefix_t[-1]!=prefix_t[-2] else observed[-1])
    ...
    out["improvement_vs_persistence"]=out["rmse_persistence"]-out["rmse_model"];out["improvement_vs_nearest"]=out["rmse_nearest"]-out["rmse_model"];out["improvement_vs_linear"]=out["rmse_linear"]-out["rmse_model"];return out

def evaluate_pca(train_data,heldout_data,keep,mean,std,cfg):
    train_z=np.vstack([transform(X,keep,mean,std) for _,X in train_data.values()]);scaler=StandardScaler().fit(train_z);pca=PCA(n_components=min(cfg.state_dim,train_z.shape[0],train_z.shape[1])).fit(scaler.transform(train_z));times,Xraw=heldout_data;Z=pca.transform(scaler.transform(transform(Xraw,keep,mean,std)))
    n_prefix=min(max(2,int(np.ceil(len(times)*cfg.prefix_fraction)),len(times)-1),len(times)-1);true=Z[n_prefix:];pers=np.repeat(Z[n_prefix-1][None,:],len(true),axis=0);linear=[]
    ...
    linear=np.asarray(linear);rm=lambda a,b:float(np.sqrt(np.mean((np.asarray(a)-np.asarray(b))**2)));a=rm(true,pers);b=rm(true,linear)
    return {"n_future_points":len(true),"rmse_model":b,"rmse_persistence":a,"rmse_nearest":np.nan,"rmse_linear":b,"improvement_vs_persistence":a-b,"improvement_vs_nearest":np.nan,"improvement_vs_linear":0.0}

def evaluate_autoencoder(train_data,heldout_data,keep,mean,std,cfg,seed):
    model,_=train_neural(train_data,keep,mean,std,cfg,seed,"markov");return evaluate_neural(model,train_data,heldout_data,keep,mean,std,cfg,"markov")
