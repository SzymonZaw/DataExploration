...
class BenchmarkConfig:
    max_genes:int=2000; state_dim:int=8; hidden_dim:int=128; epochs:int=250; lr:float=1e-3; prefix_fraction:float=.6; history_len:int=2; history_dim:int=16; dropout:float=.05
...
def transform(X,keep,mean,std): X=X[:,keep];return (np.where(np.isfinite(X),X,mean)-mean)/std
...
def train_neural(train_data,keep,mean,std,cfg,seed,mode):
    set_seed(seed);hist_len=cfg.history_len if mode=="memory" else 0;x,y,dt,h_raw=chronological_examples(train_data,keep,mean,std,hist_len);context_dim=1 if mode in {"delta_t","memory"} else 0;history_dim=cfg.history_dim if mode=="memory" else 0;model=DynamicStateModel(x.shape[1],cfg.state_dim,cfg.hidden_dim,context_dim,history_dim,cfg.dropout);opt=torch.optim.AdamW(model.parameters(),lr=cfg.lr,weight_decay=1e-4);best=float("inf");stale=0;patience=max(25,cfg.epochs//8)
...
def predict_step(model,current,dt,history,mode):
    ...
    if mode=="memory":
        raw=torch.tensor(history,dtype=torch.float32);b,t,d=raw.shape;model.eval()
...
def evaluate_neural(model,train_data,heldout_data,keep,mean,std,cfg,mode):
    ...
    for j in range(n_prefix,len(times)):
        dt=(float(times[j])-current_t)/scale;hist=np.asarray(observed[-cfg.history_len:]) if mode=="memory" else None;p=predict_step(model,current,dt,hist,mode);pred.append(p);persistence.append(observed[-1]);true.append(X[j]);nearest.append(np.mean([train_trans[ds][int(np.argmin(np.abs(tt-times[j])))] for ds,(tt,_) in train_data.items()],axis=0));linear.append(observed[-1]+((times[j]-prefix_t[-1])/(prefix_t[-1]-prefix_t[-2]))*(observed[-1]-observed[-2]) if len(prefix_t)>=2 and prefix_t[-1]!=prefix_t[-2] else observed[-1]);current=p;current_t=float(times[j])
