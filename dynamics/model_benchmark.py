...
Phase 0 protocol
----------------
All candidate models are evaluated under exactly the same LODO splits,
training-only preprocessing, observed-prefix forecasting task, baselines,
metrics, and permutation procedure. The benchmark tests whether temporal
ordering adds predictive information beyond static geometry.
"""
...
def train_autoencoder(train_data, keep, mean, std, cfg, seed):
    ...
    best = float("inf"); stale = 0; patience = max(25, cfg.epochs // 8)
    for _ in range(cfg.epochs):
        model.train(); opt.zero_grad(set_to_none=True); _, rec = model(x_all); loss=((rec-x_all)**2).mean(); loss.backward(); nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step(); value=float(loss.detach())
        if value < best - 1e-6: best, stale = value, 0
        else: stale += 1
        if stale >= patience: break
...
def _static_baselines(train_data, heldout_times, heldout_X, n_prefix, keep, mean, std):
    ...
    nearest=[]
    for target_t in heldout_times[n_prefix:]:
        candidates=[]
        for tt,xx in train_trans.values():
            idx=int(np.argmin(np.abs(tt-target_t))); candidates.append(xx[idx])
        nearest.append(np.mean(candidates,axis=0))
    nearest=np.asarray(nearest)
    if n_prefix>=2 and heldout_times[n_prefix-1]!=heldout_times[n_prefix-2]:
        slope=(heldout_X[n_prefix-1]-heldout_X[n_prefix-2])/(heldout_times[n_prefix-1]-heldout_times[n_prefix-2])
    else: slope=np.zeros_like(last)
    linear=np.asarray([last+slope*(t-heldout_times[n_prefix-1]) for t in heldout_times[n_prefix:]])
    return persistence, nearest, linear
...
def _metrics(true,pred,persistence,nearest,linear):
    ...
    if len(true)==0: return {"n_future_points":0,"rmse_model":np.nan,"rmse_persistence":np.nan,"rmse_nearest":np.nan,"rmse_linear":np.nan,"_true":true,"_pred":pred,"_persistence":persistence,"improvement_vs_persistence":np.nan,"improvement_vs_nearest":np.nan,"improvement_vs_linear":np.nan}
    rm=lambda a,b:float(np.sqrt(np.mean((np.asarray(a)-np.asarray(b))**2)))
    out={"n_future_points":len(true),"rmse_model":rm(true,pred),"rmse_persistence":rm(true,persistence),"rmse_nearest":rm(true,nearest),"rmse_linear":rm(true,linear),"_true":true,"_pred":pred,"_persistence":persistence}
...
def evaluate_dynamic(model,train_data,heldout_data,keep,mean,std,cfg,mode,order=None):
    ...
    for j in range(n_prefix,len(times)):
        ...
        pred.append(p); true.append(X[j]); current=p; current_t=float(times[j])
        if mode=="memory": history=(history+[p])[-cfg.history_len:]
    return _metrics(true,pred,persistence,nearest,linear)

def evaluate_autoencoder(model,transition,train_data,heldout_data,keep,mean,std,cfg,order=None):
...
def evaluate_pca(fitted,train_data,heldout_data,keep,mean,std,cfg,order=None):
...
def benchmark_fold(train_data,heldout_data,cfg,seed,heldout_name):
    ...
    for mode in MODELS:
        if mode=="pca": fitted=fit_pca_transition(train_data,keep,mean,std,cfg); r=evaluate_pca(fitted,train_data,heldout_data,keep,mean,std,cfg); train_loss=np.nan
        elif mode=="autoencoder":
            model,transition,train_loss=train_autoencoder(train_data,keep,mean,std,cfg,seed); fitted=(model,transition); r=evaluate_autoencoder(model,transition,train_data,heldout_data,keep,mean,std,cfg)
        else:
...
def permutation_null(runs,n_perm,seed=12345):
    ...
    for k in range(n_perm):
        ...
        for r in runs:
            ...
            if mode=="pca": q=evaluate_pca(r["fitted"],r["train_data"],pseudo,r["keep"],r["mean"],r["std"],r["cfg"])
            elif mode=="autoencoder": q=evaluate_autoencoder(r["fitted"][0],r["fitted"][1],r["train_data"],pseudo,r["keep"],r["mean"],r["std"],r["cfg"])
...
def benchmark(data,cfg,seeds):
    ...
    summary=df.groupby("model",as_index=False).agg(
        n_runs=("rmse_model","size"), mean_rmse=("rmse_model","mean"), median_rmse=("rmse_model","median"),
        ...
        q95_improvement_vs_linear=("improvement_vs_linear",lambda x:x.quantile(.95)),
    )
    pvals=[]
    ...
    summary["predictive_support"]=(summary.mean_improvement_vs_persistence>0)&(summary.q05_improvement_vs_persistence>0)&(summary.mean_improvement_vs_nearest>0)&(summary.q05_improvement_vs_nearest>0)&(summary.mean_improvement_vs_linear>0)&(summary.q05_improvement_vs_linear>0)&(summary.permutation_p<.05)
    return df,summary
