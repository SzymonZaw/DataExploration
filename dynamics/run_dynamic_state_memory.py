...
OUT=ROOT/"results"/"Dynamics"/"dynamic_state_memory"
DATASETS=["GSE67462","GSE28688","GSE297234"]
DEFAULT_SEEDS=(311,312,313,314,315)
HISTORY_LEN=2
...
def stats(train,max_genes):
    pooled=np.vstack([X for _,X in train.values()])
    med=np.nanmedian(np.where(np.isfinite(pooled),pooled,np.nan),axis=0); med=np.where(np.isfinite(med),med,0.)
    pooled=np.vstack([np.where(np.isfinite(X),X,med) for X in [X for _,X in train.values()]])
    var=np.var(pooled,axis=0); keep=np.argsort(var)[::-1][:min(max_genes,len(var))]
    mean=pooled[:,keep].mean(0); std=pooled[:,keep].std(0); std=np.where(std>1e-8,std,1.)
...
def sequence_examples(train,keep,mean,std):
    ...
    for times,Xraw in train.values():
        ...
        for i in range(HISTORY_LEN,len(times)-0):
            xs.append(X[i]); ys.append(X[i+1] if i+1<len(times) else X[i]); dts.append((tn[i+1]-tn[i]) if i+1<len(times) else 0.)
            hs.append(X[i-HISTORY_LEN:i])
        # remove the terminal example with no target
        xs=xs[:-1]; ys=ys[:-1]; dts=dts[:-1]; hs=hs[:-1]
...
def train_model(train_data,keep,mean,std,state_dim,hidden_dim,epochs,lr,seed,use_history):
    set_seed(seed); x,h,y,dt=sequence_examples(train_data,keep,mean,std)
    ...
    for _ in range(epochs):
        ...
        pred=model.transition(z,context=dt,history=h if use_history else None); pred_obs=model.decode(pred)
        loss=((rec-x)**2).mean()+.25*((pred-zn)**2).mean()+((pred_obs-y)**2).mean()
        loss.backward(); nn.utils.clip_grad_norm_(model.parameters(),1.); opt.step(); val=float(loss.detach())
        if val<best-1e-6: best=val;stale=0
        else: stale+=1
        if stale>=patience: break
    return model,best

def predict(model,current,history,dt):
    x=torch.tensor(current[None,:],dtype=torch.float32); d=torch.tensor([[float(dt)]],dtype=torch.float32)
    h=torch.tensor(history[None,:,:],dtype=torch.float32) if history is not None else None
    model.eval()
...
def evaluate(model,train_data,held,keep,mean,std,prefix_fraction,use_history):
    times,Xraw=held; X=transform(Xraw,keep,mean,std); scale=max(float(times[-1]-times[0]),1.)
    n=max(HISTORY_LEN+1,int(np.ceil(len(times)*prefix_fraction))); n=min(n,len(times)-1)
    prefix_t=list(times[:n]); prefix_X=[*X[:n]]; true=[]; pm=[]; pp=[]
    for j in range(n,len(times)):
        dt=(times[j]-times[j-1])/scale; hist=np.asarray(prefix_X[-HISTORY_LEN:]) if use_history else None
        pm.append(predict(model,prefix_X[-1],hist,dt)); pp.append(prefix_X[-1]); true.append(X[j])
        prefix_t.append(times[j]); prefix_X.append(X[j])
...
def permutation_null(data,max_genes,state_dim,hidden_dim,epochs,lr,seed,prefix_fraction,n_perm):
    ...
    for k in range(n_perm):
        ...
        for held_name in sorted(perm):
            ...
            m,_=train_model(train,keep,mean,std,state_dim,hidden_dim,max(20,epochs//3),lr,seed+1000+k,True)
            r=evaluate(m,train,perm[held_name],keep,mean,std,prefix_fraction,True); fold.append(r["improvement_vs_persistence"])
...
def run(max_genes=2000,state_dim=8,hidden_dim=128,epochs=250,lr=1e-3,seeds=DEFAULT_SEEDS,prefix_fraction=.6,n_perm=100):
    ...
    for seed in seeds:
        for held_name in sorted(data):
            ...
            for kind,use_history in (("markov",False),("memory",True)):
                model,loss=train_model(train,keep,mean,std,state_dim,hidden_dim,epochs,lr,seed,use_history)
                r=evaluate(model,train,data[held_name],keep,mean,std,prefix_fraction,use_history)
    ...
    if df.empty: raise RuntimeError("No valid memory LODO folds.")
    piv=df.pivot_table(index=["seed","heldout_dataset"],columns="model",values="rmse").reset_index(); piv["memory_gain_vs_markov"]=piv["markov"]-piv["memory"]
    summary=pd.DataFrame([{"n_valid_fold_models":len(df),"n_seeds":df.seed.nunique(),"mean_rmse_markov":df[df.model=="markov"].rmse.mean(),"mean_rmse_memory":df[df.model=="memory"].rmse.mean(),"mean_memory_gain_vs_markov":piv.memory_gain_vs_markov.mean(),"median_memory_gain_vs_markov":piv.memory_gain_vs_markov.median(),"q05_memory_gain_vs_markov":piv.memory_gain_vs_markov.quantile(.05),"q95_memory_gain_vs_markov":piv.memory_gain_vs_markov.quantile(.95),"mean_improvement_memory_vs_persistence":df[df.model=="memory"].improvement_vs_persistence.mean()}])
