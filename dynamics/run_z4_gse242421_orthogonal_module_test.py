...
def find_reference_direction(explicit: Path | None, root: Path):
    if explicit is not None and explicit.exists(): return pd.read_csv(explicit), str(explicit)
    candidates = [
        root/"results/Dynamics/z6_gse67462_temporal_modules/02_temporal_modules.csv",
        root/"results/Dynamics/z6_gse67462_temporal_modules/04_temporal_module_summary.csv",
...
def normalize_gene(x): return str(x).strip().upper()

def sample_day(x):
    m = re.search(r"D(\d+(?:\.\d+)?)", str(x), flags=re.I)
...
def direction_from_reference(ref, module):
    if ref is None or ref.empty: return None
    modcol=next((c for c in ref.columns if c.lower()=="module"),None)
    if modcol is None: return None
    ...
    for c in ["reference_direction","frozen_direction","time_spearman","gse67462_time_spearman","direction"]:
        if c in g.columns:
            ...
            if len(vals):
                ...
                if v>0:return 1.0
                if v<0:return -1.0
    if "trajectory_label" in g.columns:
        label=str(g["trajectory_label"].iloc[0]).lower()
        if label in {"late_rising","sustained"}: return 1.0
...
def main():
    ...
    a=ap.parse_args(); out=a.output; out.mkdir(parents=True,exist_ok=True); root=Path.cwd()
    for p in [a.activity,a.mapping,a.modules]:
    ...
    activity.index=activity.index.map(normalize_gene); activity=activity[~activity.index.duplicated(keep="first")]
    mapping=pd.read_csv(a.mapping)
    if not {"human_gene","mouse_gene"}.issubset(mapping.columns): raise ValueError("Frozen mapping missing human_gene/mouse_gene")
    mapping["human_gene_norm"]=mapping.human_gene.map(normalize_gene); mapping["mouse_gene_norm"]=mapping.mouse_gene.map(normalize_gene)
    ...
    if not {"module","gene"}.issubset(modules.columns): raise ValueError("Frozen module table must contain module and gene columns")
    modules["gene_norm"]=modules.gene.map(normalize_gene)
    ref,ref_path=find_reference_direction(a.reference,root)
    ...
    for module in module_ids:
        ...
        for s,v in zip(ALL_ORDER,score): score_rows.append({"module":module,"sample":s,"time_days":sample_day(s),"score":float(v),"n_genes":len(human)})
        obs=rho(time_days,score[:len(TIME_ORDER)]); null=np.array([rho(time_days,rng.permutation(score[:len(TIME_ORDER)])) for _ in range(a.permutations)])
        ref_dir=direction_from_reference(ref,module)
        if ref_dir is None: p=np.nan; concordant=np.nan
        else:
            signed_obs=ref_dir*obs; signed_null=ref_dir*null
        ...
        module_rows.append({"module":module,"frozen_mouse_genes":len(mg),"validated_human_genes":len(human),"coverage_fraction":len(human)/max(len(mg),1),"target_time_spearman":obs,"frozen_reference_direction":ref_dir,"direction_concordant":concordant,"permutation_p_directional":p,"permutations":a.permutations})
