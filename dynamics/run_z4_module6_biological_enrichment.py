...
def gprofiler(genes, organism, background, sources, insecure_ssl=False):
    payload = {"organism": organism, "query": [str(g) for g in genes], "sources": sources,
               "user_threshold": 0.05, "no_evidences": False, "combined": False}
    if background:
        payload["background"] = [str(g) for g in background]
...
def main():
    ...
    ap.add_argument("--output",type=Path,default=DEFAULT_OUT); ap.add_argument("--module",type=int,default=6)
    ap.add_argument("--core-quantile",type=float,default=0.75); ap.add_argument("--skip-gprofiler",action="store_true")
    ap.add_argument("--insecure-ssl",action="store_true"); a=ap.parse_args(); out=a.output; out.mkdir(parents=True,exist_ok=True)
    for p in [a.modules,a.mapping,a.activity]:
        if not p.exists(): raise FileNotFoundError(f"Required input missing: {p}")
    modules=pd.read_csv(a.modules); mapping=pd.read_csv(a.mapping); activity=pd.read_csv(a.activity,sep="\t",compression="gzip",index_col=0)
    activity.index=activity.index.map(norm); activity=activity[~activity.index.duplicated()]; modules["gene_norm"]=modules.gene.map(norm)
    m6_mouse=sorted(modules.loc[pd.to_numeric(modules.module,errors="coerce")==a.module,"gene_norm"].unique())
    mapping["mouse_norm"]=mapping.mouse_gene.map(norm); mapping["human_norm"]=mapping.human_gene.map(norm)
    frozen=mapping.dropna(subset=["mouse_norm","human_norm"]).drop_duplicates("mouse_norm"); mouse_to_human=dict(zip(frozen.mouse_norm,frozen.human_norm))
