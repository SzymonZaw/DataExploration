...
def _ensg_to_input_from_meta(obj, query_name):
    """Map g:Profiler Ensembl IDs in intersections back to submitted IDs.

    g:GOSt documents that intersections are aligned with the ENSG list in
    genes_metadata. The mapping object is commonly serialized as a named
    input->ENSG mapping; tolerate inverse/different JSON representations.
    """
    try:
        qmeta=obj["meta"]["genes_metadata"]["query"][query_name]
        ensgs=[str(x) for x in (qmeta.get("ensgs") or [])]
        ...
        if isinstance(mapping, dict):
            ...
            for k,v in mapping.items():
                ...
                for vv in vals:
                    vv=str(vv); kk=str(k)
                    if vv in ensgs: out[vv]=kk
                    elif kk in ensgs: out[kk]=str(v[0] if isinstance(v,list) and v else v)
            return out
        if isinstance(mapping, list) and len(mapping)==len(ensgs):
            vals=[str(v) for v in mapping]
            # If mapping is a parallel list of input IDs, use it. If it is
            # simply another copy of ENSGs, no symbol recovery is possible.
            if vals != ensgs and any(v not in set(ensgs) for v in vals):
                return dict(zip(ensgs,vals))
        # Some serializers expose an aligned input/query list.
        for key in ("input", "inputs", "query"):
            vals=qmeta.get(key)
            if isinstance(vals,list) and len(vals)==len(ensgs):
                return dict(zip(ensgs,[str(v) for v in vals]))
    except Exception:
        pass
    return {}
...
def gprofiler(genes, organism, background, sources, insecure_ssl=False):
    ...
    req=urllib.request.Request("https://biit.cs.ut.ee/gprofiler/api/gost/profile/",data=data,headers={"Content-Type":"application/json","Accept":"application/json","User-Agent":"DataExploration-Z4-Module6/1.9"},method="POST")
    ...
    for r in (obj.get("result") or []):
        ...
        if query_name not in mapping_cache:
            mapping_cache[query_name]=_ensg_to_input_from_meta(obj,query_name)
        ensg_to_input=mapping_cache[query_name]
        qmeta=(obj.get("meta",{}).get("genes_metadata",{}).get("query",{}).get(query_name,{}) or {})
        ensgs=[str(x) for x in (qmeta.get("ensgs") or [])]
        genes_hit=[]
        if len(intersections)==len(ensgs):
            for i,item in enumerate(intersections):
                if item not in (None, [], ""):
                    ensg=ensgs[i]
                    genes_hit.append(ensg_to_input.get(ensg,ensg))
