...
OUT=ROOT/"results"/"Dynamics"/"stage2_10"; OUT.mkdir(parents=True,exist_ok=True)
TARGET=["GSE67462","GSE28688","GSE297234"]; GRID=np.linspace(0,1,40); N_BOOT=500; N_PERM=1000
PROGRAMS={"P01_PLURIPOTENCY":["POU5F1","SOX2","NANOG","LIN28A","LIN28B","DPPA4","UTF1","ESRRB","KLF4","MYC"],"P02_PROLIFERATION":["MKI67","PCNA","TOP2A","CCNB1","CCNB2","CCNE1","CDK1","CDC20","UBE2C","TYMS","MCM2","MCM3","MCM5","MCM6","MCM7"],"P03_EMT_MESENCHYMAL":["VIM","ZEB1","ZEB2","SNAI1","SNAI2","TWIST1","FN1","ITGA5","COL1A1","COL1A2","CDH2","CDH1","EPCAM","KRT8","KRT18","KRT19"],"P04_STRESS_RESPONSE":["DDIT3","ATF4","HSPA1A","HSPA1B","HMOX1","XBP1","JUN","FOS","DUSP1","PPP1R15A","DNAJB1"],"P05_GLYCOLYTIC_METABOLISM":["SLC2A1","HK2","PFKP","ALDOA","GAPDH","ENO1","PKM","LDHA","PGK1","TPI1","PDK1"],"P06_FGFR_PI3K_MAPK":["FGFR1","FGFR2","FGFR3","FGFR4","FRS2","PLCG1","PIK3CA","PIK3CB","AKT1","AKT2","MAPK1","MAPK3","RAF1","SOS1"],"P07_CHROMATIN_EPIGENETIC":["KMT2A","KMT2B","EZH2","DNMT1","DNMT3A","DNMT3B","TET1","TET2","HDAC1","HDAC2","SMARCA4","ARID1A","CHD4","SUZ12"],"P08_ECM_ADHESION":["FN1","ITGA5","ITGB1","COL1A1","COL1A2","COL3A1","SPARC","VCAN","THBS1","LAMC1","LAMA4"]}
def log(x): print(f"Stage 2.10: {x}",flush=True)
def finite(X,fill=None):
 X=np.asarray(X,float).copy()
 if fill is None: fill=np.array([np.median(v[np.isfinite(v)]) if np.isfinite(v).any() else 0.0 for v in X.T])
 else: fill=np.where(np.isfinite(np.asarray(fill,float)),np.asarray(fill,float),0.0)
 bad=~np.isfinite(X)
 if bad.any(): ii=np.where(bad); X[ii]=fill[ii[1]]
 return X,fill
def normtime(t):
 t=np.asarray(t,float); lo,hi=np.min(t),np.max(t); return np.zeros_like(t) if hi<=lo else (t-lo)/(hi-lo)
...
def load():
 ...
 for ds in TARGET:
  ...
  X=matrix.loc[:,g.matrix_column.astype(str)].T.copy(); X.index=g.time_hours.to_numpy(float); X=X.groupby(level=0,sort=True).mean(); out[ds]=(X.index.to_numpy(float),X.to_numpy(float),[str(x).upper() for x in matrix.index]); log(f"{ds}: {len(g)} timed samples -> {len(X)} unique timepoints")
 return out
...
def spearman(a,b): return float(pd.Series(a).corr(pd.Series(b),method="spearman")) if len(a)>=3 else np.nan
def fold(held,train_names,traj):
 train={d:traj[d] for d in train_names}; ht,hX,hgenes=traj[held]; fill=train_fill(train); train_times={}
