...
def _program_grid(traj, grid=17):
    ...
    u = np.linspace(0.0, 1.0, grid); tn = (t-t[0])/(t[-1]-t[0])
...
def _project_progress(traj,model):
    ...
    x=traj.to_numpy(float);med=np.nanmedian(x,axis=0);x=np.where(np.isfinite(x),x,med);z=model["sign"]*model["pca"].transform(model["scaler"].transform(x))[:,0]
...
def _fold(matrix,meta,held_out,n_programs=10,max_genes=2500,seed=42,grid=17):
    ...
    if held_out not in te or len(tr)<2:return None,None,{"reason":"insufficient_trajectories","n_train_trajectories":len(tr)}
    cube,used=_profile_cube(tr);stab=_stability(cube);ids,selected,min_ds=_select(stab,max_genes)
    if len(ids)<max(2,n_programs):return None,stab,{"reason":"too_few_stable_genes","n_train_trajectories":len(used),"n_stable_genes":len(ids),"min_datasets":min_ds}
    ...
    if model is None or test is None:return None,stab,{"reason":"no_progress_model","n_train_trajectories":len(used),"n_stable_genes":len(ids)}
    pred=_project_progress(test,model);actual=(test.index.to_numpy(float)-test.index.min())/(test.index.max()-test.index.min())
    if pred is None or len(pred)!=len(actual) or len(pred)<3:return None,stab,{"reason":"too_few_test_timepoints","n_test_timepoints":0 if pred is None else len(pred)}
    valid=np.isfinite(actual)&np.isfinite(pred)
    if valid.sum()<3:return None,stab,{"reason":"insufficient_finite_progress","n_test_timepoints":int(valid.sum())}
    a,p=actual[valid],pred[valid];spearman=float(pd.Series(a).corr(pd.Series(p),method="spearman"));pear=float(np.corrcoef(a,p)[0,1]) if np.std(p)>1e-12 else np.nan;rmse=float(np.sqrt(np.mean((a-p)**2)));mono=float(np.mean(np.diff(p)>=-0.02)) if len(p)>1 else np.nan
    row={"held_out_dataset":held_out,"n_timepoints":len(p),"n_training_trajectories":len(used),"n_stable_genes":len(ids),"n_programs":len(programs),"min_datasets_for_selection":min_ds,"mean_gene_stability":float(selected.median_pairwise_correlation.head(len(ids)).mean()),"progress_spearman":spearman,"progress_pearson":pear,"progress_rmse":rmse,"progress_monotonicity":mono,"train_pc1_time_correlation":model["train_pc1_time_correlation"]}
    ...
    state_rows=[]
    for ds,f in trajs.items():
        q=f.reset_index().rename(columns={"index":"time_hours"});q.insert(0,"dataset",ds);q.insert(0,"held_out_dataset",held_out);state_rows.append(q)
    state_path=pd.concat(state_rows,ignore_index=True) if state_rows else pd.DataFrame()
    return {"summary":pd.DataFrame([row]),"trajectory":path,"state_trajectory":state_path},stab,{"reason":"ok","n_train_trajectories":len(used),"n_stable_genes":len(ids),"min_datasets":min_ds}
...
def stage2_9_2(n_programs=10,max_genes=2500,permutations=20,seed=42,grid=17):
    print("Stage 2.9.2: loading common gene space...",flush=True);matrix,meta=_load_common_space();datasets=sorted(meta.dataset.astype(str).unique());observed=[];trajectories=[];state_trajectories=[];statuses=[]
    for i,ds in enumerate(datasets,1):
        print(f"Stage 2.9.2: fold {i}/{len(datasets)}, held out={ds}...",flush=True);result,_,status=_fold(matrix,meta,ds,n_programs,max_genes,seed,grid);status.update({"held_out_dataset":ds,"mode":"observed"});statuses.append(status)
        if result is None:print(f"  skipped: {status}",flush=True)
        else:observed.append(result["summary"]);trajectories.append(result["trajectory"]);state_trajectories.append(result["state_trajectory"])
    summary=pd.concat(observed,ignore_index=True) if observed else pd.DataFrame();summary.to_csv(OUT/"01_latent_progress_summary.csv",index=False);(pd.concat(trajectories,ignore_index=True) if trajectories else pd.DataFrame()).to_csv(OUT/"02_latent_progress_trajectories.csv",index=False);pd.DataFrame(statuses).to_csv(OUT/"05_fold_status.csv",index=False);(pd.concat(state_trajectories,ignore_index=True) if state_trajectories else pd.DataFrame()).to_csv(OUT/"04_program_state_trajectories.csv",index=False)
    null=[]
    for p in range(permutations):
        pm=_permute_meta(meta,seed+1000+p);vals=[]
        for ds in datasets:
            result,_,_=_fold(matrix,pm,ds,n_programs,max_genes,seed+1000+p,grid)
            if result is not None:vals.append(result["summary"])
        if vals:
