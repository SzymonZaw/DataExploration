"""Stage 2.9.24: leakage-free delta-t-aware one-step state dynamics.

Tests whether the latent state transition depends on the elapsed time between
observations. No ODE/state-space model is fitted here.
"""
...
OUT = ROOT / "results" / "Dynamics" / "stage2_9_24"
...
def log(x):
    print(f"Stage 2.9.24: {x}", flush=True)
...
def design(meta):
    ...
    t = times(meta); ok = np.isfinite(t); tn = np.zeros_like(t)
...
def projection(matrix, meta, train_ds, test_ds, genes):
    tr = meta[meta.dataset.astype(str).isin(train_ds)].copy()
    allm = meta[meta.dataset.astype(str).isin(list(train_ds) + [test_ds])].copy()
    ...
    Xtr = np.array(matrix.loc[genes, trc].T.to_numpy(float), copy=True)
    Xa = np.array(matrix.loc[genes, ac].T.to_numpy(float), copy=True)
    med = np.nanmedian(Xtr, axis=0); med = np.where(np.isfinite(med), med, 0.0)
    for X in (Xtr, Xa):
        bad = ~np.isfinite(X)
        if bad.any(): X[bad] = np.take(med, np.where(bad)[1])
    scaler = StandardScaler().fit(Xtr)
    pca = PCA(n_components=min(10, Xtr.shape[0] - 1, Xtr.shape[1]), random_state=0).fit(scaler.transform(Xtr))
...
def delta_t_model(train_pairs, test_pairs):
    dt_scale = np.median([r[4] for r in train_pairs])
    if not np.isfinite(dt_scale) or abs(dt_scale) < 1e-12: dt_scale = 1.0
    def design_rows(rows):
        return np.asarray([[r[1], r[4] / dt_scale, r[1] * (r[4] / dt_scale)] for r in rows], float)
    X = design_rows(train_pairs); Y = np.asarray([r[3] for r in train_pairs], float)
    model = LinearRegression().fit(X, Y)
    return model, design_rows(test_pairs), dt_scale


def evaluate(matrix, meta, heldout, variant, genes):
    train_ds = [d for d in TARGET if d != heldout]
    traj = projection(matrix, meta, train_ds, heldout, genes)
    ...
    if len(train_pairs) < 4 or len(test_pairs) < 2:
    ...
    model, Xte, dt_scale = delta_t_model(train_pairs, test_pairs)
    pred = model.predict(Xte); true = np.asarray([r[3] for r in test_pairs]); persistence = np.asarray([r[1] for r in test_pairs])
    train_traj = [traj[d] for d in train_ds if d in traj]
    nearest_pred = np.asarray([nearest_training_prediction(train_traj, r[2]) for r in test_pairs])
    linear_pred = np.asarray([linear_time_prediction(train_traj, r[2]) for r in test_pairs])
    good = np.isfinite(pred)&np.isfinite(true)&np.isfinite(persistence)&np.isfinite(nearest_pred)&np.isfinite(linear_pred)
    if good.sum() < 2:
    ...
    row = {"status":"ok","held_out_dataset":heldout,"variant":variant,"n_genes":len(genes),"n_training_transitions":len(train_pairs),"n_test_transitions":int(good.sum()),"mean_test_delta_hours":float(np.mean([r[4] for r in test_pairs])),"future_state_rmse":rm(pred),"persistence_rmse":rm(persistence),"nearest_time_rmse":rm(nearest_pred),"linear_time_rmse":rm(linear_pred),"improvement_vs_persistence":rm(persistence)-rm(pred),"improvement_vs_nearest_time":rm(nearest_pred)-rm(pred),"improvement_vs_linear_time":rm(linear_pred)-rm(pred),"future_state_pearson":corr(pred[good],true[good]),"future_state_spearman":corr(pred[good],true[good],"spearman"),"delta_t_scale_hours":dt_scale,"coef_current_state":float(model.coef_[0]),"coef_delta_t":float(model.coef_[1]),"coef_state_x_delta_t":float(model.coef_[2]),"intercept":float(model.intercept_)}
    preds=[]
    for r,p,n,l in zip(test_pairs,pred,nearest_pred,linear_pred):
        preds.append({"held_out_dataset":heldout,"variant":variant,"current_time_hours":r[0],"future_time_hours":r[2],"delta_hours":r[4],"current_state":r[1],"true_future_state":r[3],"predicted_future_state":p,"persistence_prediction":r[1],"nearest_training_time_prediction":n,"linear_time_prediction":l})
    return row, preds
...
def permutation(matrix, meta, heldout, variant, genes, seed):
    ...
    train_pairs=training_pairs(traj,train_ds); test_pairs=one_step(*traj[heldout])
    if len(train_pairs)<4 or len(test_pairs)<2:return []
    model,Xte,_=delta_t_model(train_pairs,test_pairs);pred=model.predict(Xte);true=np.asarray([r[3] for r in test_pairs]);obs=float(np.sqrt(np.mean((pred-true)**2)));rng=np.random.default_rng(seed);null=np.empty(N_PERM)
...
def run():
    ...
    for fold,heldout in enumerate(TARGET,1):
        ...
        for v in VARIANTS:
            log(f"  {v}: {len(sets[v])} training-selected genes"); r,pr=evaluate(matrix,meta,heldout,v,sets[v]); folds.append(r); preds.extend(pr); perms.extend(permutation(matrix,meta,heldout,v,sets[v],24000+fold))
    f=pd.DataFrame(folds); p=pd.DataFrame(preds); s=pd.concat(selections,ignore_index=True) if selections else pd.DataFrame()
    perm_cols=["held_out_dataset","variant","observed_future_state_rmse","permutation_p_rmse","null_mean_rmse","null_p05_rmse","null_p95_rmse"]
    pm=pd.DataFrame(perms,columns=perm_cols); summaries=[]
    for v in VARIANTS:
        g=f[(f.variant==v)&(f.status=="ok")]; row={"variant":v,"n_valid_folds":len(g)}
        for k in ("future_state_rmse","persistence_rmse","nearest_time_rmse","linear_time_rmse","improvement_vs_persistence","improvement_vs_nearest_time","improvement_vs_linear_time","future_state_pearson","future_state_spearman","coef_current_state","coef_delta_t","coef_state_x_delta_t"):
        ...
        q=pm[pm["variant"]==v] if "variant" in pm.columns else pd.DataFrame(columns=perm_cols); row["min_permutation_p_rmse"]=float(q.permutation_p_rmse.min()) if len(q) else np.nan; row["mean_permutation_p_rmse"]=float(q.permutation_p_rmse.mean()) if len(q) else np.nan; summaries.append(row)
