...
def finite(X, fill=None):
    ...
    if fill is None:
        med = np.full(X.shape[1], np.nan, float)
        valid = np.isfinite(X)
        for j in range(X.shape[1]):
            if valid[:, j].any(): med[j] = np.median(X[valid[:, j], j])
...
def resample(t, X, fill=None):
    tn = normalize_time(t); X, med = finite(X, fill)
...
def baseline_scale(Y, baseline=None, amp=None):
    Y, _ = finite(Y); baseline = Y[0].copy() if baseline is None else np.asarray(baseline,float)
    Z = Y-baseline
    if amp is None:
...
def corr(a,b,method="pearson"):
    ...
    if a.shape != b.shape: raise ValueError(f"Stage 2.9.32 corr shape mismatch: {a.shape} vs {b.shape}")
...
def load_trajectories():
    ...
    for ds in TARGET:
        g=meta[meta["dataset"].astype(str).eq(ds)].copy(); g["time_hours"]=pd.to_numeric(g["time_hours"],errors="coerce")
        g=g[g.time_hours.notna()&g.matrix_column.notna()]
        if g.time_hours.nunique()<3: continue
        cols=g.matrix_column.astype(str).tolist(); X=matrix.loc[:,cols].T.copy(); X.index=g.time_hours.to_numpy(float)
        X=X.groupby(level=0,sort=True).mean(); out[ds]=(X.index.to_numpy(float),X.to_numpy(float),list(matrix.index))
...
def select_shared_genes(train, all_genes):
    ...
    A=np.stack(states); mean=A.mean(axis=0); shared=np.var(mean,axis=0); hetero=np.mean((A-mean[None,:,:])**2,axis=(0,1))
    score=shared/(shared+hetero+1e-12); order=np.argsort(score)[::-1]; chosen=order[score[order]>=0.55]
...
def fit_shared(train, all_genes):
    genes,scores=select_shared_genes(train,all_genes); pos={str(g):i for i,g in enumerate(all_genes)}; idx=[pos[str(g)] for g in genes]; states=[]
    for t,X,_ in train.values(): states.append(baseline_scale(resample(t,X[:,idx])[0])[0])
    stack=np.vstack(states); pca=PCA(n_components=1,random_state=2932).fit(stack); pcs=[pca.transform(s).ravel() for s in states]; shared=np.mean(np.stack(pcs),axis=0)
...
def program_activity(Y, genes):
    Y,_=finite(Y)
    frame=pd.DataFrame(Y.T,index=[str(g).upper() for g in genes]).rank(axis=0,pct=True); out={}
...
def prepare_fold(heldout, train_names, traj):
    ...
    prefix_raw, med=resample(t[:-1],X[:-1,idx]); prefix_state,base,amp=baseline_scale(prefix_raw); full_raw,_=resample(t,X[:,idx],med); full_state,_,_=baseline_scale(full_raw,base,amp)
    prefix_pc=pca.transform(prefix_state).ravel(); true_pc=float(pca.transform(full_state)[-1,0]); prefix_shared=shared.copy()
    agreement_p=corr(prefix_pc,prefix_shared); agreement_s=corr(prefix_pc,prefix_shared,"spearman"); residual=prefix_pc-shared; residual_final=float(residual[-1])
    pred_shared=float(shared[-1]); pred_dataset=pred_shared+residual_final; persistence=float(prefix_pc[-1]); nearest=float(shared[-1]); acts=program_activity(prefix_state,genes); biological=[]
    for pid,a in acts.items(): biological.append({"program_id":pid,"component_program_pearson":corr(prefix_pc,a),"component_program_spearman":corr(prefix_pc,a,"spearman")})
...
def evaluate(a):
    true=a["true_pc"]; rmse_d=abs(a["pred_dataset"]-true); rmse_s=abs(a["pred_shared"]-true); rmse_p=abs(a["persistence"]-true); rmse_n=abs(a["nearest"]-true)
...
def permutation_null(artifacts,n=N_PERM):
    ...
    for b in range(n):
        ...
        for di,(ds,a) in enumerate(artifacts.items()):
            rng=np.random.default_rng(932000+b*17+di); perm=a["prefix_pc"][rng.permutation(len(a["prefix_pc"]))]; pred=float(a["shared"][-1]+np.mean(perm-a["shared"])); err=abs(pred-a["true_pc"]); per=abs(a["persistence"]-a["true_pc"]); vals.append(per-err)
...
def run():
    ...
    for held in names:
        train=[d for d in names if d!=held]; log(f"LODO held out {held}; training on {', '.join(train)}"); a=prepare_fold(held,train,traj)
    ...
    R.to_csv(OUT/"01_lodo_shared_biological_invariance.csv",index=False); B=pd.DataFrame(bio_rows); B.to_csv(OUT/"02_biological_program_association.csv",index=False); P=permutation_null(artifacts); P.to_csv(OUT/"03_time_permutation_null.csv",index=False)
    obs=float(R.improvement_vs_persistence.mean()); nv=P.mean_improvement_vs_persistence.dropna().to_numpy(float); p=float((1+np.sum(nv>=obs))/(len(nv)+1)) if len(nv) else np.nan
    mean_agree=float(R.agreement_spearman.mean()); mean_resid=float(R.residual_rms.mean()); bio_strength=float(B.component_program_spearman.abs().mean()) if len(B) else np.nan; support=bool(mean_agree>0.5 and obs>0 and np.isfinite(p) and p<0.05 and bio_strength>0.3)
    ...
    summary.to_csv(OUT/"04_stage2932_summary.csv",index=False); log("overall:"); print(summary.to_string(index=False)); return summary
