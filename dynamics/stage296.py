...
def _project(sc,pc,sign,ref,y):
    y=np.asarray(y,float); med=np.nanmedian(y,axis=0); y=np.where(np.isfinite(y),y,med); y=np.where(np.isfinite(y),y,0)
...
def run(min_fold_recurrence=2,seed=42):
    global MATRIX,META,MEM
    MATRIX,META,MEM=_load()
    folds=sorted(MEM["held_out_dataset"].astype(str).unique())
    # Program labels are not comparable across KMeans folds. Gene recurrence
    # is therefore used as the consensus-program proxy without forcing labels.
    rec=MEM.groupby("gene")["held_out_dataset"].nunique().sort_values(ascending=False)
    genes2=rec[rec>=2].index.astype(str).tolist()
    ...
    for ds in folds:
        ...
        rr=train_mem.groupby("gene")["held_out_dataset"].nunique()
        genes=rr[rr>=min_fold_recurrence].index.astype(str).tolist()
        ...
        ax=_axis(train,genes); test=_trajectory(MATRIX,META,ds,genes)
        ...
        sc,pc,sign,ref=ax; tt,y=test; z=_project(sc,pc,sign,ref,y)
        target_train=np.tile(np.linspace(0,1,9),len(ref)//9)
        rows.append({"held_out_dataset":ds,"status":"ok","n_consensus_genes":len(genes),"train_axis_time_pearson":_corr(ref,target_train,"pearson"),"progress_spearman":_corr(tt,z,"spearman"),"progress_pearson":_corr(tt,z,"pearson"),"progress_rmse":float(np.sqrt(np.mean((tt-z)**2))),"progress_monotonicity":float(np.mean(np.diff(z)>=-.02))})
        if len(genes3)>=20:
