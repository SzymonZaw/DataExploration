...
def _log(msg): print(f"Stage 2.9.7: {msg}",flush=True)
...
def _load():
    _log("loading common gene matrix, metadata and recurrence...")
    m=pd.read_csv(COMMON/"06_common_human_gene_matrix.csv",index_col=0).apply(pd.to_numeric,errors="coerce"); meta=pd.read_csv(COMMON/"07_common_gene_sample_metadata.csv"); rec=pd.read_csv(IN/"02_gene_recurrence.csv"); _log(f"loaded {m.shape[0]:,} genes x {m.shape[1]:,} samples; {len(rec):,} recurrence records"); return m,meta,rec
...
def _stable_genes(train,recurrence=2,min_temporal_corr=.5,max_genes=1000):
    _log(f"selecting stable genes from {len(train)} training datasets (recurrence >= {recurrence}, temporal |rho| >= {min_temporal_corr})...")
    ...
    for i,g in enumerate(sorted(genes),1):
        ...
        if i%250==0:_log(f"stable-gene scan: {i:,}/{len(genes):,}")
    vals=sorted(vals,key=lambda x:(x[1],x[2]),reverse=True)[:max_genes];_log(f"stable-gene selection complete: {len(vals):,} genes retained");return [x[0] for x in vals],pd.DataFrame(vals,columns=["gene","mean_abs_temporal_spearman","n_training_datasets"])

def _modules(genes,n_modules=8):
    _log(f"building deterministic consensus modules from {len(genes):,} genes...")
    ...
    for i,g in enumerate(genes,1):
        ...
        if i%250==0:_log(f"module profile construction: {i:,}/{len(genes):,}")
    if len(used)<n_modules:return None
    score=np.mean(np.asarray(profiles)*np.linspace(-1,1,9),axis=1);order=np.argsort(score);chunks=np.array_split(order,min(n_modules,len(order)));mods={f"module_{i+1:02d}":[used[j] for j in ch] for i,ch in enumerate(chunks) if len(ch)};_log(f"built {len(mods)} modules");return mods
...
def _fit_axis(train,modules):
    _log("fitting latent consensus axis on training datasets only...")
    frames=[]
    for ds in train:
    ...
    X=np.concatenate(frames);sc=StandardScaler().fit(X);pc=PCA(n_components=1,random_state=42).fit(sc.transform(X));z=pc.transform(sc.transform(X))[:,0];target=np.tile(np.linspace(0,1,9),len(frames));r=_corr(z,target,"pearson");_log(f"training axis fit complete: Pearson(time, axis)={r:.3f}");return sc,pc,(1 if np.isfinite(r) and r>=0 else -1),z,r
...
def run(recurrence=2,min_temporal_corr=.5,n_modules=8,max_genes=1000):
    ...
    _log(f"{len(folds)} datasets detected: {', '.join(folds)}")
    for fold_i,ds in enumerate(folds,1):
        TRAIN=[x for x in folds if x!=ds];_log(f"FOLD {fold_i}/{len(folds)}: held out {ds}; training on {len(TRAIN)} datasets")
        genes,diag=_stable_genes(TRAIN,recurrence,min_temporal_corr,max_genes);diag.insert(0,"held_out_dataset",ds);stable_rows.append(diag);modules=_modules(genes,n_modules);test=_trajectory(ds,genes)
        if modules is None or test is None:rows.append({"held_out_dataset":ds,"status":"insufficient_consensus_modules","n_stable_genes":len(genes),"n_modules":0});_log(f"FOLD {fold_i}/{len(folds)}: skipped — insufficient consensus modules/test trajectory");continue
        for name,gs in modules.items():
            for g in gs:membership.append({"held_out_dataset":ds,"module":name,"gene":g})
        axis=_fit_axis(TRAIN,modules)
        if axis is None:rows.append({"held_out_dataset":ds,"status":"insufficient_training_state","n_stable_genes":len(genes),"n_modules":len(modules)});_log(f"FOLD {fold_i}/{len(folds)}: skipped — insufficient training state");continue
        ...
        row={"held_out_dataset":ds,"status":"ok","n_stable_genes":len(genes),"n_modules":len(modules),"train_axis_time_pearson":train_corr,"progress_spearman":_corr(tn,zn),"progress_pearson":_corr(tn,zn,"pearson"),"progress_rmse":float(np.sqrt(np.mean((tn-zn)**2))),"progress_monotonicity":float(np.mean(np.diff(zn)>=-.02))};rows.append(row);_log(f"FOLD {fold_i}/{len(folds)} complete: Spearman={row['progress_spearman']:.3f}, Pearson={row['progress_pearson']:.3f}, RMSE={row['progress_rmse']:.3f}")
