...
human gene space. Replicate holdout and a time-permutation null are reported as
secondary controls.
"""
...
def _trajectories(matrix, metadata, time_override=None):
    ...
    for ds, g in metadata.groupby("dataset"):
        g = g[g.matrix_column.isin(matrix.columns) & g.time_hours.notna()].copy()
        if len(g) < 2:
            continue
...
def leave_one_replicate_out(matrix, metadata):
    ...
    for ds, g in metadata.groupby("dataset"):
        ...
        if len(reps) < 2:
            continue
        for held in reps:
            train = g[g.replicate != held].copy()
            test = g[g.replicate == held].copy()
