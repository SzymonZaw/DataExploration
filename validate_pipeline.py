...
from dynamics.stage26 import stage2_6_robust
...
def _recover_legacy_metadata():
    """Repair old Stage 2.6 metadata whose matrix columns are deterministic labels."""
...
def _write_dataset_roles():
    """Classify datasets by actual timed support; single-timepoint sets are context-only."""
    if not COMMON_METADATA.exists():
        return pd.DataFrame()
    metadata = pd.read_csv(COMMON_METADATA)
    from Dynamics import GSM_TIME, condition, replicate, time_hours
    rows = []
    for ds, g in metadata.groupby("dataset", sort=True):
        times = []
        for sample in g["sample"].astype(str):
            times.append(time_hours(ds, sample))
        timed = pd.Series(times).dropna()
        unique = sorted(set(float(x) for x in timed))
        ...
        rows.append({
            ...
            "time_hours": ",".join(map(str, unique)),
            "role": role,
        })
    out = pd.DataFrame(rows)
    out.to_csv(ROOT / "results" / "Dynamics" / "stage2_7" / "00_dataset_roles.csv", index=False)
    print("\nStage 2.7 dataset roles:")
    print(out.to_string(index=False))
    return out


if __name__ == "__main__":
    print("Running robust Stage 2.6 refresh...")
    ...
    if refresh.get("status") != "sufficient_common_human_gene_space":
        if COMMON_MATRIX.exists() and COMMON_METADATA.exists():
            print("WARNING: new Stage 2.6 mapping is insufficient; continuing with the last valid common-space files.")
        else:
